...
class DatabaseWriter(PipelineQueryMixin):
    ...
    async def execute_transaction(self, queries_and_params: List[Tuple[str, Any]], timeout: float = 30.0) -> Any:
        """
        Executes a sequence of queries inside a single atomic transaction.
        """
        loop = asyncio.get_running_loop()
        future = loop.create_future()
        await self.queue.put(("__TRANSACTION__", queries_and_params, future))

        try:
            return await asyncio.wait_for(future, timeout=timeout)
        except asyncio.TimeoutError as te:
            logger.error("Transaction execution timed out", query_count=len(queries_and_params))
            raise te
